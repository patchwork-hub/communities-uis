...
`.trim();
  ...
  isCommunitiesActive = (match, location) => {
    return match || location.pathname.startsWith('/communities');
  };`.trim();
  replaceContent(activeFirehost, activeCommunities, targetFilePath);
...
<ColumnLink transparent to='/communities/all' icon='community' isActive={this.isCommunitiesActive} iconComponent={CommunityIcon} activeIconComponent={CommunityActiveIcon} text={intl.formatMessage(messages.community)} />
</>`;
  ...
  console.log(
    chalk.green("Added community menu item and updated file successfully.")
  );
};
